...
| 类型 | 支持的常见格式 | 行为 |
| --- | --- | --- |
| 视频 | MP4、MKV、MOV、AVI、WebM、FLV | 优先检查匹配源语言的内嵌文本字幕，否则运行 ASR |
| 音频 | MP3、WAV、M4A、FLAC、AAC、OGG | 始终运行 ASR |
| 字幕 | SRT | 跳过 ASR，直接翻译 |

ASS、VTT 和 LRC 当前是输出格式，不是可用字幕输入。文件选择后，应用会先检查媒体和字幕来源；检查完成前不能开始任务。
...
- **使用术语库**：点击主窗口任务句中的入口，可创建和维护独立术语库，并勾选本任务要使用的集合。任务开始后使用冻结快照，因此后续编辑集合不会改变旧任务。
...
如果需要换翻译模型重新处理已经保存的识别稿，在“任务处理”中使用“重新翻译”。它会按当前翻译设置创建新任务，不重新运行语音识别，并会产生新的翻译模型调用。

取消后的任务如果存在有效进度点，任务处理窗仍可能提供“继续任务”。强制关闭进程不会让已经保存的阶段自动消失，但最后一个尚未完成的阶段可能需要重新执行。

## 9. 审看、编辑与重新导出

任务完成后，主窗口提供“审看结果”“打开字幕”“打开文件夹”和“重新导出”。“审看结果”会进入任务处理窗的字幕工作区。

### 9.1 任务处理窗

任务处理窗可以按制作中、待处理、待校对、已完成和已取消筛选任务。选择任务后，根据其状态显示可用动作，例如继续、取消、重新翻译、打开任务目录、打开结果目录或重新导出。
...
存在未保存字幕修改时，关闭任务处理窗或离开当前结果会弹出确认。放弃修改后无法从当前编辑状态恢复。
...
这是预期行为。继续任务必须沿用原任务快照。需要换模型时，在任务处理窗使用“重新翻译”，从已经保存的识别稿创建新翻译任务。
...
打开“任务处理”，选择对应任务。文件被移动或删除时使用“重新导出”；目录不可写时选择新的输出目录。已经完成的识别和翻译不需要重跑。
