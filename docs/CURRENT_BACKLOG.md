...
| FFmpeg 自建 core 与公开分发闭环 | 待验收 | 无可选外部库的 core 已接入标准 runtime，并通过完整媒体矩阵、portable RC、包内 Local Service、可见启动和 internal NSIS payload 验证；当前正式 pin 尚未切换。仍需发布不可变候选资产，在干净 Windows/用户环境完成最终 installer 与真实片源验收，并完成许可审查后才能采用及将 `public_distribution_ready` 提升为 true | [`APP_RUNTIME.md`](APP_RUNTIME.md) |
