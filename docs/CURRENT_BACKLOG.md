...
| OpenRouter 云 ASR 真实服务验收 | 待验收 | 2026-07-27 已用用户配置的 key 完成极省用量的短音频验证：Whisper 返回可用 segment，Grok 返回准确文本但未透出 words/segments，三次请求合计约 0.000188 美元；平台错误/重试、两个专项 profile、中断不丢失的任务级 usage 汇总、普通 key 用量/限额查询和 Flutter 完整/部分用量区分已具备，剩余长音频、多语言与真实内容人工验收 | [`KNOWN_ISSUES_AND_VALIDATION.md`](KNOWN_ISSUES_AND_VALIDATION.md) |
