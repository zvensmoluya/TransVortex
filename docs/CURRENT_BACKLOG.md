...
| OpenRouter 云 ASR 真实服务验收 | 待验收 | 2026-07-28 已确认 Whisper segment 与 Grok `verbose_json + word` 时间戳链路；Grok 现以 300 秒窗口、3 秒 overlap 先合并词时间轴再生成字幕段，正常窗口与细分重试均有无文本诊断。平台错误/重试、两个专项 profile、中断不丢失的任务级 usage 汇总、普通 key 用量/限额查询和 Flutter 完整/部分用量区分已具备，剩余真实长音频、多语言、切句质量与窗口阈值人工验收 | [`KNOWN_ISSUES_AND_VALIDATION.md`](KNOWN_ISSUES_AND_VALIDATION.md) |
