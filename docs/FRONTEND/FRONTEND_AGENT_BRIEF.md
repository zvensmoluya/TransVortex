...
- 保存状态和导出状态必须分开显示。
- 做视觉 reset 或重新设计时，必须遵守 `FRONTEND_DESIGN_STYLE.md` 的“视觉重设计约束”：结构先于颜色、中性色优先、状态色小面积使用、灰度也要成立、工作区而不是卡片墙、禁止卡片嵌套卡片、阴影默认不用、图标有等级。
- 视觉品质是验收门槛。前端必须明确呈现“低光字幕制作工作台”气质，不能做成通用深色 admin dashboard、SaaS 控制台、参数配置后台或组件 demo。
