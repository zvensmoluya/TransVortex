...
当前另有一个最小只读诊断工具窗，供开发、测试和支持使用，不进入普通用户菜单。它展示 `desktop.snapshot.environment` 的 doctor 报告及任务上下文，相关实现和 smoke 继续保留。任务处理窗是任务历史 / 详情 / 结果编辑的新主入口；跨任务批量筛查、完整编辑器、完整历史恢复矩阵和高级导出复核仍属后续。
...
- worker checkpoint 已累计真实 started 模型请求并按术语初始化、分片翻译、术语更新、格式恢复、批量补回、单行修复和质量处理分类；模型返回与本地拆分不重复计数。主窗口运行态显示累计请求，任务处理窗显示分类制作摘要；Release 本地完整 E2E 已从真实任务状态读到请求统计，`taskProcessing` browse smoke 已验证 14 次夹具请求的分类摘要与零 overflow 截图。
...
- 内部诊断工具窗继续可由测试和支持入口启动并读取 Local Service doctor 报告；它不是普通用户完成任务的必做界面。
...
- 术语管理完整页。
- 诊断结果到修复动作的完整页。
- 公开分发签名、FFmpeg 对应源码托管、干净 Windows 真实任务和已安装路径通知中心最终行为；当前 NSIS 安装器已包含固定 Python / FFmpeg runtime，并通过本机安装、升级、运行中保护、AUMID 快捷方式和卸载自动验收，但内部产物仍明确 `public_release_ready=false`。
- 高级打包 / 托盘。
...
- `powershell -ExecutionPolicy Bypass -File scripts\smoke_flutter_release_matrix.ps1` 可一次性导出 release 主流程完成态、完成态通知检查、主窗口六态、4 个非主窗口基础 case、`taskProcessing` 编辑 / 失败线索 / 恢复 / 取消四个追加 case 和长模型名设置窗的 Flutter 渲染树截图，并保存每个 case 的 JSON 报告与矩阵 summary；summary 会记录渲染尺寸、非背景采样、Flutter overflow 警告条采样、任务处理窗选中状态、编辑 / 重新导出 / 失败线索 / 恢复 / 取消结果、失败线索字段（错误码 / 阶段 / 重试性 / 可继续）、诊断窗 / 任务处理窗结果目录可写性和通知调用 / registry 结果，用于复查截图里暴露过的布局 / overflow、任务处理窗主流程和通知接线；summary 也会写入 `frontend_design_mvp_complete=false`、自动化覆盖范围和仍需人工验收清单，防止把自动 smoke 绿灯误读成“前端设计 MVP 完成”
...
- `powershell -ExecutionPolicy Bypass -File scripts\install_flutter_desktop_shortcut.ps1` 可为开发 / portable 路径创建和校验 `TransVortex.Desktop` AUMID 快捷方式；NSIS 安装器由 native runner 命令写入同一身份，并已在安装 / 升级 / 卸载验收中核对。release smoke 仍保留人工完整端到端边界，避免把自动通知或快捷方式检查误读成整体完成。
- `powershell -ExecutionPolicy Bypass -File scripts\build_app_runtime.ps1 -Force` 构建固定主 runtime；`scripts\build_ffmpeg_runtime.ps1 -Force` 构建固定 FFmpeg 8.1 LGPL shared runtime；`scripts\package_flutter_release.ps1 -OutputRoot <dir> -LaunchCheck` 生成同时包含二者的 portable release 包，验证 Local Service RPC、FFmpeg / FFprobe 版本及二进制 / DLL 哈希。portable manifest 仍明确 `installer=false`、`native_installer=false`、`installer_format_complete=false`，不能替代 NSIS 原生安装器。
- `powershell -ExecutionPolicy Bypass -File scripts\install_flutter_portable_release.ps1 -SourceRoot <portable-package> -InstallRoot <dir> -ShortcutPath <lnk> -Force` 可把 portable 包复制到用户级安装目录，创建 / 校验 AUMID 快捷方式，并在安装目录复跑固定 Local Service 和 FFmpeg 检查；包根 `Install-TransVortex.ps1` 是同一能力的解压后入口，只用于 portable 分发。
- `powershell -ExecutionPolicy Bypass -File scripts\build_windows_installer.ps1 -AllowUnsigned -Force` 生成不含终端用户 `.ps1` 的 NSIS 内部验收安装包；默认用户级安装，具备 staging / 回滚、升级清旧文件、运行进程保护、AUMID 快捷方式和卸载注册。没有签名证书时必须显式传入 `-AllowUnsigned`，manifest 会保持 `public_release_ready=false`。
- `powershell -ExecutionPolicy Bypass -File scripts\accept_windows_installer.ps1` 自动验证全新安装、固定 Python Local Service、受限 `PATH` 下 bundled FFmpeg / FFprobe、升级清旧文件、可见启动和 mutex、运行中安装 / 卸载拦截、快捷方式身份、正常卸载及用户数据保留；本机当前安装器已通过。公开发布仍需 Authenticode 签名、FFmpeg 完整对应源码托管和干净 Windows 环境真实媒体任务验收。
...
- 真实可见窗口诊断：`scripts\smoke_flutter_release_matrix.ps1 -CheckDesktopComposite` 当前矩阵覆盖 17 个 release case，包含主流程完成态、完成态通知、主窗口六态、4 个非主窗口基础 case、`taskProcessing` edit / failure / resume / cancel 和长模型名设置窗。本机已通过 17 case release matrix 的 Flutter 渲染树截图、桌面合成层截图和两类 overflow 采样，17/17 个 case 写出有效桌面合成层截图，其中 `taskProcessing_failure` summary 记录 `smoke_detail_resumable`、`TRANSLATE`、可重试和可继续线索。脚本会前置窗口并校验桌面截图像 TransVortex 浅色工作区，避免误抓背后窗口。这补了一层真实窗口区域证据，但还没有覆盖用户手动选择片源 / 提交 / 观察运行 / 完成 / 打开结果 / 审看结果的完整人工路径；该人工路径现在由 `scripts\accept_flutter_release_manual.ps1` 固化为可记录的 JSON 验收流程。
...
- 完成态能打开结果。
- 真实可见 release 窗口端到端完成前，只能宣称“Flutter MVP 接线成立”，不得宣称“前端设计 MVP 完成”；人工验收脚本 `-LaunchCheck` 已确认本机 release 窗口可启动并可写初始截图，但完整 G1 仍需人工操作通过。固定 Python / FFmpeg runtime 和 NSIS 安装、升级、运行中保护、快捷方式、卸载已自动验收；真实外部翻译 / 语音识别已在本机跑通，但其他用户环境仍需复跑，公开发布还需签名、FFmpeg 对应源码托管和干净机验证。
...
8. 待验收：运行 `scripts\accept_flutter_release_manual.ps1` 完成真实可见窗口完整任务端到端人工验收，并在正式安装目录和干净 Windows 环境重跑精简任务；NSIS 安装器、固定 Python / FFmpeg、升级和卸载自动验收已经通过。系统通知真实横幅已由人工确认 release 目录路径可用，但仍需验证已安装路径通知归属；真实外部翻译 / 语音识别需在目标用户环境复跑；公开产物另需签名和 FFmpeg 对应源码地址。
...
| 主窗口提交任务 | 已接 `runtime.submitRun`；默认 `output_dir` 指向片源同目录，输出目录不可写时可选择新目录并重试；翻译未配置时会阻塞全部翻译任务，音频 / 视频在识别配置不可用时阻塞，SRT 直译跳过 ASR 校验；真实 Python Local Service 子进程 smoke 已覆盖 submit；release exe smoke 已覆盖主窗口 controller 提交路径，使用临时本地翻译服务完成 `video_asr_translate` 到 `DONE`；本机外部服务 smoke 已用真实 provider 完成 `DemoTest/英文视频.mp4` 到 `DONE` | 后续补音频独立端到端证据、更完整失败修复矩阵和真实可见窗口人工验收 |
...
| 系统通知 | 已接 `flutter_local_notifications` Windows Toast；仅在任务从运行态进入完成 / 失败且窗口不在前台时触发，有 observer / widget 防回归覆盖，点击回调聚焦主窗口，release 包已包含通知 DLL；Windows runner 已设置进程级 AUMID；`-CheckNotifications` release smoke 已覆盖 native 初始化 / show 调用 / AUMID registry 注册和 Windows Notifications Settings key；NSIS 快捷方式目标和 AUMID 已在安装 / 升级 / 卸载验收中核对；用户已人工确认系统横幅出现；payload 不携带完整源文件路径 | 后续补已安装路径下的通知中心归属和点击聚焦；非 MSIX 下插件文档说明通知历史和取消能力不可用 |
...
截至当前实现，主窗口真实任务流、两个设置窗、诊断工具窗和任务处理窗的 **MVP 接线**已经成立；release smoke、17 case 窗口矩阵、真实外部服务 smoke，以及 NSIS 安装 / 升级 / 卸载验收分别提供了代码链路、可见窗口、外部服务和安装拓扑证据。剩余关键证明是人工完整任务链路、其他目标用户环境复跑、已安装路径真实任务与通知、干净 Windows、签名和 FFmpeg 对应源码托管。通过这些边界前，不应宣称“前端设计 MVP 已完成”或“公开发布就绪”。
