...
不得在修复动作未接线前放假按钮；当前只允许跳转到已真实接线的翻译模型设置 / 语音识别设置、打开 doctor 报告里的产物目录路径，或对 `can_resume` 任务调用真实 `runtime.submitResume`。完整诊断修复台、运行队列操作、完整历史恢复矩阵、完整结果编辑器、任务详情诊断和按任务输出目录专项诊断延后。
