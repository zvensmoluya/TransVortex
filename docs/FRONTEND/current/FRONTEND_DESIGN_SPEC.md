...
- 真实可见窗口仍是独立验收项：`scripts\smoke_flutter_release_matrix.ps1 -CheckDesktopComposite` 可覆盖当前 15 个 release case；矩阵包含主流程完成态、完成态通知、主窗口六态、4 个非主窗口基础 case、`taskProcessing` edit / resume 和长模型名设置窗，本机已通过不带桌面合成层采样的 15 case release render matrix。这补足了自动窗口区域截图证据，但不覆盖人工选择片源 / 提交 / 观察运行 / 完成 / 打开结果 / 审看结果的完整路径；该路径用 `scripts\accept_flutter_release_manual.ps1` 记录逐步确认、窗口截图和 JSON 报告，通过 G1 前仍要做人工直接观察。
