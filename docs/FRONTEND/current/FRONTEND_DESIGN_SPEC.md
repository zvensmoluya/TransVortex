...
- 真实可见窗口仍是独立验收项：`scripts\smoke_flutter_release_matrix.ps1 -CheckDesktopComposite` 可覆盖当前 18 个 release case；本机已通过不带桌面合成层采样的 18 case release render matrix，此前也验证过旧 15 个 case 的 Flutter 渲染树截图、桌面合成层截图和 JSON 报告，且 `desktop_composite_ok=true`、`desktop_composite_overflow_stripe_samples=0`；新增 `taskProcessing` browse / edit / resume 已通过单窗口 release render smoke。这补足了自动窗口区域截图证据，但不覆盖人工选择片源 / 提交 / 观察运行 / 完成 / 打开结果 / 审看结果的完整路径；该路径用 `scripts\accept_flutter_release_manual.ps1` 记录逐步确认、窗口截图和 JSON 报告，通过 G1 前仍要做人工直接观察。
