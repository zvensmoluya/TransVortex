...
- 历史任务能查看状态和事件；可恢复任务已有最小继续动作，可取消任务已有最小取消动作；任务目录和结果目录已有最小打开动作。任务处理窗的完整恢复矩阵仍属后续。
...
- 当前 `scripts\smoke_flutter_release_matrix.ps1 -CheckDesktopComposite` 已用 ffmpeg `ddagrab` 记录 Windows 桌面合成层窗口区域；完整矩阵定义覆盖 16 个 release case，包含主流程完成态、完成态通知、主窗口六态、4 个非主窗口基础 case、`taskProcessing` edit / resume / cancel 和长模型名设置窗；本机已通过带桌面合成层采样的 15 case release matrix，15/15 个 case 写出桌面合成层截图，Flutter 渲染树与桌面合成层 overflow 警告条采样均为 0，通知 show、结果编辑重导出和失败任务恢复动作仍为通过；新增的 `taskProcessing` cancel case 需随下次矩阵复跑刷新桌面合成层证据。该项补充真实窗口区域证据，但不替代 G1 的人工选择片源 / 提交 / 观察运行 / 完成或取消 / 打开结果 / 审看结果。
