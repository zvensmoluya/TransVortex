...
- `scripts\build_app_runtime.ps1` 构建固定 Windows Embedded Python 主 runtime、安装 TransVortex wheel 和锁定依赖。`scripts\package_flutter_release.ps1` 是当前 portable release 包入口：它把 Flutter release bundle、主 runtime、`prompts/`、`pipeline.yaml`、由 `providers.example.yaml` 复制出的 `providers.yaml`、`README_PORTABLE.txt`、用户级安装脚本和开始菜单快捷方式辅助脚本打包到同一目录，并排除 `.env`、`providers.local.yaml`、`auth.json` 等本地凭据文件；脚本默认从包根启动 `runtime\python\python.exe -m transvortex.app_service --no-pump`，在空 `PYTHONPATH` 下验证 `service.info` / `service.health` / `asr.status` / `service.shutdown` 及 runtime 版本；`-LaunchCheck` 额外从包目录启动 `TransVortex.exe`。`scripts\install_flutter_portable_release.ps1` 和包根 `Install-TransVortex.ps1` 可把包复制到用户级安装目录、创建 AUMID 快捷方式并在安装目录复跑 Local Service RPC。manifest 写入 `frontend_design_mvp_complete=false`，并明确 `installer=false`、`formal_installer=false`、`python_runtime_included=true`、`ffmpeg_included=false`；该包不是正式 MSIX/MSI/NSIS/Inno 安装器。
