...
- 当前 `scripts\smoke_flutter_release_matrix.ps1 -CheckDesktopComposite` 可用 ffmpeg `ddagrab` 记录 Windows 桌面合成层窗口区域；矩阵脚本当前覆盖 16 个 release case（完成态、完成态通知、主窗口六态、7 个非主窗口和长模型名设置窗），其中旧 15 个 case 已验证 `desktop_composite_ok=true`，新增 `taskProcessing` 已通过单窗口 release render smoke（含渲染树截图和 overflow 检查）。该项补充真实窗口区域证据，但不替代 G1 的人工选择片源 / 提交 / 观察运行 / 完成或取消 / 打开结果 / 审看结果。
