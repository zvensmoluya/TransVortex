...
- 当前 `scripts\smoke_flutter_release_matrix.ps1 -CheckDesktopComposite` 可用 ffmpeg `ddagrab` 记录 Windows 桌面合成层窗口区域；完整矩阵当前覆盖 18 个 release case，其中旧 15 个 case 已验证 `desktop_composite_ok=true`，新增 `taskProcessing` browse / edit / resume 已通过单窗口 release render smoke，`-SkipCompletedTask` 矩阵已验证 16 个非完整主流程 case。该项补充真实窗口区域证据，但不替代 G1 的人工选择片源 / 提交 / 观察运行 / 完成或取消 / 打开结果 / 审看结果。
