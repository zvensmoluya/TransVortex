...
- 任务处理窗已有工作台入口，可通过真实 `tasks.list` 展示任务片列、按全部 / 制作中 / 待处理 / 已完成筛选任务、选中任务预览、最近事件、完成任务内嵌编辑、失败任务继续动作和运行中任务取消动作，并打开任务目录 / 结果目录；旧任务历史窗和任务详情窗已移除，旧 `taskHistory` / `taskDetail` 启动 ID 会兼容进入任务处理窗。
