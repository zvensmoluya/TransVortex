...
- 任务处理窗已有工作台入口，可通过真实 `tasks.list` 展示任务片列、选中任务预览、最近事件、完成任务内嵌编辑和失败任务继续动作，并打开任务目录 / 结果目录；旧任务历史窗和任务详情窗已移除，旧 `taskHistory` / `taskDetail` 启动 ID 会兼容进入任务处理窗。
...
- 旧最小任务历史窗和任务详情窗已移除；任务历史、任务详情、任务目录 / 结果目录打开和失败任务继续动作由任务处理窗承载。
...
- 当前 `scripts\smoke_flutter_release_matrix.ps1 -CheckDesktopComposite` 已用 ffmpeg `ddagrab` 记录 Windows 桌面合成层窗口区域；完整矩阵覆盖 15 个 release case，包含主流程完成态、完成态通知、主窗口六态、4 个非主窗口基础 case、`taskProcessing` edit / resume 和长模型名设置窗；本机已通过带桌面合成层采样的 15 case release matrix，15/15 个 case 写出桌面合成层截图，Flutter 渲染树与桌面合成层 overflow 警告条采样均为 0，通知 show、结果编辑重导出和失败任务恢复动作仍为通过。该项补充真实窗口区域证据，但不替代 G1 的人工选择片源 / 提交 / 观察运行 / 完成或取消 / 打开结果 / 审看结果。
