...
- 完成态已经能读取结果、进入最小结果审看窗、编辑并保存片段译文、选择输出格式 / 单双语重新导出、打开字幕、打开所在文件夹；打开前会检查结果文件是否仍在原位置，缺失时进入“重新导出”修复态；重新导出时若输出目录不可写，可选择新目录对同一任务重新导出。
...
- 任务历史已有独立最小工具窗，可通过真实 `tasks.list` 展示最近任务，从完成任务打开最小结果审看窗，并打开任务目录 / 结果目录；任务详情已有独立最小工具窗，可通过真实 `tasks.list` / `tasks.events` 展示任务上下文和事件流，并打开任务目录 / 结果目录。
...
- 完成态可以进入结果审看入口。
- 当前已有最小结果审看窗，可通过真实 `result.open` 展示任务摘要、片段、输出格式和问题提示，可按全部 / 有问题 / 空译文筛查片段，通过 `result.segments.save` 保存片段译文，并通过 `result.reexport` 按用户选择的输出格式 / 单双语重新导出当前结果；后续再补跨任务批量筛查、完整编辑器和高级导出复核。
...
- 任务历史有入口，但不抢占主窗口主视角。
- 当前已有最小任务历史窗，使用真实 `tasks.list` 展示任务状态、片源、输出格式和失败摘要；完成任务可进入最小结果审看，所有历史任务可进入任务详情，并可打开任务目录 / 结果目录。
- 当前已有最小任务详情窗，使用真实 `tasks.events` 展示事件流，能打开任务目录 / 结果目录，并对后端标记 `can_resume` 的任务调用真实 `runtime.submitResume` 继续任务；任务详情 / 事件不做主窗口常驻日志面板。
- 历史任务能查看状态和事件；可恢复任务已有最小继续动作；任务目录和结果目录已有最小打开动作。更完整的恢复矩阵仍属后续。
...
- 当前 `scripts\smoke_flutter_release_matrix.ps1 -CheckDesktopComposite` 可用 ffmpeg `ddagrab` 记录 Windows 桌面合成层窗口区域；本机已验证 15 个 release case 的 `desktop_composite_ok=true`，覆盖完成态、完成态通知、主窗口六态和六个工具窗，桌面合成 PNG 路径保存在矩阵输出目录，且 `desktop_composite_overflow_stripe_samples=0`。该项补充真实窗口区域证据，但不替代 G1 的人工选择片源 / 提交 / 观察运行 / 完成或取消 / 打开结果 / 审看结果。
