...
- `powershell -ExecutionPolicy Bypass -File scripts\smoke_flutter_release.ps1 -CheckDesktopComposite` 可额外用 ffmpeg `ddagrab` 抓 Windows 桌面合成层窗口区域，作为真实可见窗口诊断；`smoke_flutter_release_matrix.ps1 -CheckDesktopComposite` 当前矩阵定义覆盖 16 个 release case，本机已通过带桌面合成层采样的 15 case release matrix，15/15 个 case 写出桌面合成层截图，Flutter 渲染树与桌面合成层 overflow 警告条采样均为 0；新增的 `taskProcessing` cancel case 需随下次矩阵复跑刷新桌面合成层证据。该项仍只证明窗口区域被桌面合成层捕获，不替代人工完整操作验收。
...
- 真实可见窗口诊断：`scripts\smoke_flutter_release_matrix.ps1 -CheckDesktopComposite` 当前矩阵定义覆盖 16 个 release case，包含主流程完成态、完成态通知、主窗口六态、4 个非主窗口基础 case、`taskProcessing` edit / resume / cancel 和长模型名设置窗；本机已通过带桌面合成层采样的 15 case release matrix，15/15 个 case 写出桌面合成层截图，Flutter 渲染树与桌面合成层 overflow 警告条采样均为 0，通知 show、结果编辑重导出和失败任务恢复动作仍为通过；新增的 `taskProcessing` cancel case 需随下次矩阵复跑刷新桌面合成层证据。这补了一层真实窗口区域证据，但还没有覆盖用户手动选择片源 / 提交 / 观察运行 / 完成 / 打开结果 / 审看结果的完整人工路径；该人工路径现在由 `scripts\accept_flutter_release_manual.ps1` 固化为可记录的 JSON 验收流程。
