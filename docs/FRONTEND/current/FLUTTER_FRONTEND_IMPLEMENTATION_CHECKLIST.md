...
- `powershell -ExecutionPolicy Bypass -File scripts\smoke_flutter_release.ps1 -CheckDesktopComposite` 可额外用 ffmpeg `ddagrab` 抓 Windows 桌面合成层窗口区域，作为真实可见窗口诊断；`smoke_flutter_release_matrix.ps1 -CheckDesktopComposite` 当前完整矩阵可覆盖 18 个 release case，本机此前已覆盖旧 15 个 case，新增 `taskProcessing` browse / edit / resume 已通过单窗口 release render smoke，`-SkipCompletedTask` 矩阵已验证 16 个非完整主流程 case。该项仍只证明窗口区域被桌面合成层捕获，不替代人工完整操作验收。
...
- 真实可见窗口诊断：`scripts\smoke_flutter_release_matrix.ps1 -CheckDesktopComposite` 当前完整矩阵可覆盖 18 个 release case；本机此前已验证旧 15 个 case，报告 `desktop_composite_ok=true`、桌面合成 PNG 路径有效，完成态任务为 `DONE`，完成态通知分支 `notification_check_ok=true`，且 `desktop_composite_overflow_stripe_samples=0`；新增 `taskProcessing` browse / edit / resume 已通过单窗口 release render smoke，`-SkipCompletedTask` 矩阵已验证 16 个非完整主流程 case。这补了一层真实窗口区域证据，但还没有覆盖用户手动选择片源 / 提交 / 观察运行 / 完成 / 打开结果 / 审看结果的完整人工路径；该人工路径现在由 `scripts\accept_flutter_release_manual.ps1` 固化为可记录的 JSON 验收流程。
