...
- 完成态已经能读取结果、进入任务处理窗内嵌结果审看 / 编辑工作台、按全部 / 有问题 / 空译文筛查片段、按源文 / 译文 / 问题提示搜索片段、编辑并保存片段译文、选择输出格式 / 单双语重新导出，并打开字幕 / 所在文件夹；目标文件缺失时会进入重新导出修复态；重新导出失败且输出目录不可写时可选择新目录对同一任务重新导出；错误修复件深联动、完整历史恢复矩阵、术语管理、跨任务批量筛查和完整结果编辑器仍属后续轮次。
