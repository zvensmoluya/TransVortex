...
- `powershell -ExecutionPolicy Bypass -File scripts\smoke_flutter_release.ps1 -CheckDesktopComposite` 可额外用 ffmpeg `ddagrab` 抓 Windows 桌面合成层窗口区域，作为真实可见窗口诊断；`smoke_flutter_release_matrix.ps1 -CheckDesktopComposite` 当前完整矩阵可覆盖 15 个 release case，本机已通过不带桌面合成层采样的 15 case release render matrix。该项仍只证明窗口区域被桌面合成层捕获，不替代人工完整操作验收。
...
- 真实可见窗口诊断：`scripts\smoke_flutter_release_matrix.ps1 -CheckDesktopComposite` 当前完整矩阵可覆盖 15 个 release case，包含主流程完成态、完成态通知、主窗口六态、4 个非主窗口基础 case、`taskProcessing` edit / resume 和长模型名设置窗；本机已通过不带桌面合成层采样的 15 case release render matrix。这补了一层真实窗口区域证据，但还没有覆盖用户手动选择片源 / 提交 / 观察运行 / 完成 / 打开结果 / 审看结果的完整人工路径；该人工路径现在由 `scripts\accept_flutter_release_manual.ps1` 固化为可记录的 JSON 验收流程。
