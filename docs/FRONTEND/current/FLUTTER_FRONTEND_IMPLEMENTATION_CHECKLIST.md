...
- `powershell -ExecutionPolicy Bypass -File scripts\build_app_runtime.ps1 -Force` 构建固定主 runtime；`scripts\build_ffmpeg_runtime.ps1 -Force` 构建固定 FFmpeg 8.1 LGPL shared runtime；`scripts\package_flutter_release.ps1 -OutputRoot <dir> -LaunchCheck` 生成同时包含二者的 portable release 包，验证 Local Service RPC、FFmpeg / FFprobe 版本及二进制 / DLL 哈希。portable manifest 仍明确 `installer=false`、`formal_installer=false`，不能替代正式安装器。
