...
- Grok 默认最长上传窗口暂设 300 秒，并配 3 秒 overlap：OpenRouter 公布的是约 60 秒上游“处理时间”超时，而不是 60 秒音频上限；按上述短样本处理倍率外推，300 秒约保留 2 倍处理时间余量。项目上传的 16 kHz 单声道 PCM WAV 在 300 秒时约 9.2 MiB，亦低于 multipart 的 25 MB 量级。窗口长度和 overlap 都是当前工程估算，不是 xAI 或 OpenRouter 的官方推荐或保证；真实长内容、不同语言、网络条件和负载尚未验收，因此 Grok 仍标为“实验性”。正常窗口和超时后的细分重试共用同一套词级 overlap 合并。
