...
- `openrouter_stt` 使用 OpenRouter 的 `/api/v1/audio/transcriptions` JSON 接口，音频会编码为 base64 放入 `input_audio` 上传。OpenRouter 也支持 OpenAI-compatible 的 `multipart/form-data`：它把模型、语言等参数作为普通表单字段，把音频作为二进制 `file` 字段上传，不是流式识别。当前 TransVortex 仍使用 JSON base64，不会在两种传输间自动切换。桌面端从用户级凭据中的 `openrouter_asr` 读取密钥，并以 `OPENROUTER_API_KEY` 作为开发兼容兜底；Provider YAML 不保存密钥。
...
- `x-ai/grok-stt-1.0` 的 xAI 原生 API 文档列出了 word timestamps、说话人分离和多声道能力，但 OpenRouter 当前标准 STT 响应没有公布这些字段的归一化结构。2026-07-27 对同一段 3.447 秒合成英文音频的真实请求只返回准确的 `text + usage`，没有 `words` 或 `segments`。因此首版仍标为“实验性”，按约 20 秒短窗生成粗时间轴；实现已能在响应实际出现 `words` 时将其保留为统一 `word_timestamps` meta，但在真实响应出现并稳定验证前，不能宣称具备 Grok 精确字幕、说话人或多声道支持。该承载层与 FunASR 的统一 `Segment` / meta 思路相同，响应字段仍由 OpenRouter 适配器单独解析，不能直接套用 FunASR 结构。
