...
- Desktop：Tauri + React 开发工作台，已具备环境检查、provider/key 配置、任务启动、事件读取、历史任务、结果编辑和重导出等开发态能力。
...
- Translation Memory、Subtitle Refine Agent、Visual Context、多厂商 ASR Gateway 仍属于增强方向，不阻塞当前 V1.x。
...
- **R4（产品化）**：Windows 安装包、FFmpeg 检测或内置策略、默认 CPU/int8 ASR、模型与依赖分发策略。
- **R5（后续增强）**：Translation Memory、Subtitle Refine Agent、Visual Context、多厂商 ASR Gateway。
...
- Python 3.10+
- FFmpeg / FFprobe：每台需要跑视频 ASR 的机器都要安装并加入 PATH；未来产品化版本应提供检测或内置策略。
- 本地 ASR 依赖：`faster-whisper`（`python -m pip install -e .[asr]`）。如果只跑 SRT/segments 翻译链路，可以不安装本地 ASR 依赖。
- Provider API Key：通过环境变量或 `.env` 提供，例如 `TVX_MODEL_API_KEY`；不要提交真实 key。
- 可选：NVIDIA CUDA 环境（用于本地 Whisper 加速）。CPU/int8 仍应作为稳定默认路径。

---
本说明书描述当前 V1.x 开发版的产品和架构基线。更细的实现清单、测试口径和短期优先级见 `docs/IMPLEMENTATION_PLAN.md`。
...
### 14.3 UI 方向
- 当前桌面端走 Tauri 跨平台应用，而不是一开始做 Web 后端。
- UI 使用 Web 技术栈承载显示层：React + TypeScript + Vite。
...
### 14.6 当前事件协议
worker 通过 JSONL 输出结构化事件，供 CLI/Tauri UI 消费：
...
- R3：Windows 安装包，内置或自动检测 FFmpeg，默认 CPU/int8 ASR，明确模型文件和 Python worker 分发策略。
- R4：macOS CPU 版；Linux 和 CUDA 作为后续高级目标。
- R5：按实际需求推进 Translation Memory、Subtitle Refine Agent、Visual Context 和多厂商 ASR Gateway。
